        ...
        if lines[idx].strip() == '<<<<<<< HEAD':
            ...
            while idx < len(lines):
                if lines[idx].strip() == '=======':
                    ...
                    idx += 1
            while idx < len(lines):
                if lines[idx].strip() == '>>>>>>> aospa/master':
                    idx += 1
                    break
                else:
                    idx += 1
...
def diff_union_lines(conf):
    merge_from_path = './platforms/platform_frameworks_base'
    custom_path = './platforms/aospa'
    confs = dict()
...
def custom_merge():
    merge_commit_base = './history/android_base/csvs/aospa'
    ...
    for idx, merge_file in enumerate(csvs):
        if merge_file.endswith('.csv') and '.android.' not in merge_file:
            ...
            merge_out = os.path.join('./history/android_base/mergetypes/aospa/', merge_file[:-4] + '-merge.csv')
    ...
    conflict_info_out = os.path.join('./history/android_base/mergetypes/aospa/', 'conflict_info.csv')
